...
local AzyroXFolder = "AzyroX"
local ConfigurationFolder = AzyroXFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'AzyroX Keybind'},
		-- buildwarnings
		-- rayfieldprompts
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("AzyroX | Function failed with error: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if callSafely(isfolder, AzyroXFolder) then
				if callSafely(isfile, AzyroXFolder..'/settings'..ConfigurationExtension) then
					file = callSafely(readfile, AzyroXFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"rayfieldOpen":{"Value":"K","Type":"bind","Name":"AzyroX Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"AzyroX Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
			...
			-- If no settings saved, apply overridden settings only
				...
				for settingName, settingValue in overriddenSettings do
					...
					assert(#split == 2, "AzyroX | Invalid overridden setting name: " .. settingName)
	...
	if not success then 
		if writefile then
			warn('AzyroX had an issue accessing configuration saving capability.')
		end
	end
end
...
local AzyroX = useStudio and script.Parent:FindFirstChild('AzyroX') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local rayfieldDestroyed = false -- True when AzyroXLibrary:Destroy() is called

repeat
	if AzyroX:FindFirstChild('Build') and AzyroX.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('AzyroX | Build Mismatch')
		print('AzyroX may encounter issues as you are running an incompatible interface version ('.. ((AzyroX:FindFirstChild('Build') and AzyroX.Build.Value) or 'No Build') ..').\n\nThis version of AzyroX is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, AzyroX = AzyroX, useStudio and script.Parent:FindFirstChild('AzyroX') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

AzyroX.Enabled = false

if gethui then
	AzyroX.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(AzyroX)
	AzyroX.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	AzyroX.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == AzyroX.Name and Interface ~= AzyroX then
			Interface.Enabled = false
			Interface.Name = "AzyroX-Old"
		end
	end
elseif not useStudio then
...
if AzyroX.AbsoluteSize.X < minSize.X and AzyroX.AbsoluteSize.Y < minSize.Y then
...
local Main = AzyroX.Main
local MPrompt = AzyroX:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = AzyroX:FindFirstChild('Drag')
...
AzyroX.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/AzyroX/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = AzyroX.Notifications
local keybindConnections = {} -- For storing keybind connections to disconnect when AzyroX is destroyed

local SelectedTheme = AzyroXLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = AzyroXLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	AzyroX.Main.BackgroundColor3 = SelectedTheme.Background
	AzyroX.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	AzyroX.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	AzyroX.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	AzyroX.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	AzyroX.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	AzyroX.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		AzyroX.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		AzyroX.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(AzyroX:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("AzyroX | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("AzyroX | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('AzyroX had an issue decoding the configuration file, please try delete the file and reopen AzyroX.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(AzyroXLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("AzyroX | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--AzyroXLibrary:Notify({Title = "AzyroX Flags", Content = "AzyroX was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(AzyroXLibrary.Flags) do
...
function AzyroXLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('AzyroX | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			AzyroXLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			AzyroXLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping " .. tostring(getSetting("General", "rayfieldOpen")) .. ".", Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		callSafely(writefile, AzyroXFolder..'/settings'..ConfigurationExtension, encoded)
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('AzyroX Settings', 0, true)

	if TabList['AzyroX Settings'] then
		TabList['AzyroX Settings'].LayoutOrder = 1000
	end

	if Elements['AzyroX Settings'] then
		Elements['AzyroX Settings'].LayoutOrder = 1000
	end
...
function AzyroXLibrary:CreateWindow(Settings)
	if AzyroX:FindFirstChild('Loading') then
		if getgenv and not getgenv().rayfieldCached then
			AzyroX.Enabled = true
			AzyroX.Loading.Visible = true

			task.wait(1.4)
			AzyroX.Loading.Visible = false
		end
	end
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				AzyroXLibrary:Notify({Title = 'Build Mismatch', Content = 'AzyroX may encounter issues as you are running an incompatible interface version ('.. ((AzyroX:FindFirstChild('Build') and AzyroX.Build.Value) or 'No Build') ..').\n\nThis version of AzyroX is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "AzyroX"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "AzyroX Interface Suite" then
		LoadingFrame.Version.Text = "AzyroX UI"
	end
	...
	if not Settings.DisableAzyroXPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				AzyroXLibrary:Notify({
					Title = "AzyroX Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		ensureFolder(AzyroXFolder.."/Discord Invites")

		if callSafely(isfile, AzyroXFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				callSafely(writefile, AzyroXFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"AzyroX RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		ensureFolder(AzyroXFolder.."/Key System")
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("AzyroX | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AzyroX specific development.')
				end
			end
		end
		...
		if callSafely(isfile, AzyroXFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				local savedKeys = callSafely(readfile, AzyroXFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						callSafely(writefile, AzyroXFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						AzyroXLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called AzyroXLibrary:Destroy()
				...
				if not Success then
					...
					print("AzyroX | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AzyroX specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= AzyroXLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					AzyroXLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			AzyroX.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("AzyroX | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AzyroX specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					AzyroXLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("AzyroX | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with AzyroX specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					AzyroXLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("AzyroX | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with AzyroX specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					AzyroXLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= AzyroXLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("AzyroX | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AzyroX specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						AzyroXLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("AzyroX | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with AzyroX specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					AzyroXLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			AzyroXLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			AzyroXLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('AzyroX had an issue creating settings.') end
...
function AzyroXLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function AzyroXLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function AzyroXLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['AzyroX Settings'])
		end)
	end)
...
function AzyroXLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				AzyroXLibrary:Notify({Title = "AzyroX Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			AzyroXLibrary:Notify({Title = "AzyroX Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('AzyroX Configurations Error | '..tostring(result))
			AzyroXLibrary:Notify({Title = "AzyroX Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = AzyroXLibrary:CreateWindow({
	--	Name = "AzyroX Example Window",
	--	LoadingTitle = "AzyroX Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using AzyroX may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like AzyroX to get the key from
	...
	----AzyroXLibrary:Notify({Title = "AzyroX Interface", Content = "Welcome to AzyroX. These - are the brand new notification design for AzyroX, with custom sizing and AzyroX calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(AzyroXLibrary.Theme) do
...
task.delay(4, function()
	AzyroXLibrary.LoadConfiguration()
